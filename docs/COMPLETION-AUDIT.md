...
| Apps and checks | `runtime.py`, `checks.py`: trusted local run; check graph with cache; merge refuses without a passing receipt | OS isolation for untrusted code; GitHub workflow marketplace import; a large production suite on this Mac |
...
| Releases | `releases.py`: bound local/http destinations; publish is incomplete until the destination serves the version; rollback is a second observed publish | Cloudflare Workers production adapter; schema/function deploys; live user-flow proof on a bound https destination |
...
| Collaboration | `integrate.py`: disjoint merge, overlapping restack, stacked parent lanes; GitHub remotes block computed absence | Issues, signed peer identity, proposal bundles between Macs, GitHub import-then-archive |
