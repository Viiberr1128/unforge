...
export default function Project({ id, back, onUpdated, onNavigationBlocked, onOpenProject }) {
  ...
  const [filePage, setFilePage] = useState(null), [fileBusy, setFileBusy] = useState(false);
  const [fileData, setFileData] = useState(null), [fileError, setFileError] = useState('');
  const [readBusy, setReadBusy] = useState(false), [fileRefresh, setFileRefresh] = useState(0);
  const [adoption, setAdoption] = useState(null);
  const [historyPage,setHistoryPage] = useState(null), [historyBusy,setHistoryBusy] = useState(false);
  useEffect(() => {let active = true;api(`/projects/${id}/history`).then(value => {if (active) setHistoryPage(value);}).catch(e => {if (active) setError(e.message);});return () => {active = false;};},[id,project]);
  async function olderHistory() {setHistoryBusy(true);setError('');try {const value = await api(`/projects/${id}/history?cursor=${historyPage.nextCursor}&limit=100`);setHistoryPage(current => ({...value,history:[...current.history,...value.history].filter((item,index,items) => items.findIndex(other => other.id === item.id) === index)}));} catch(e) {setError(e.message);} finally {setHistoryBusy(false);}}

  const [draftStatus,setDraftStatus] = useState(''), [storedDrafts,setStoredDrafts] = useState([]);
  const draftRevision = useRef(null), draftQueue = useRef(Promise.resolve()), draftTimer = useRef(null), latestContent = useRef('');
  latestContent.current = content;
  function persistDraft(value) {
    clearTimeout(draftTimer.current);
    const action = draftQueue.current.catch(() => {}).then(async () => {
      const saved = await api(`/projects/${id}/draft`,{path:file,content:value,baseContent:fileData.content,revision:draftRevision.current});
      draftRevision.current = saved.revision;
      if (latestContent.current === value) setDraftStatus('Draft saved on this Mac');
      return saved;
    });
    draftQueue.current = action;
    return action;
  }
  useEffect(() => {
    if (!draft || !fileData) return;
    setDraftStatus('Saving draft…');
    draftTimer.current = setTimeout(() => {persistDraft(content).catch(e => {setDraftStatus('Draft is not saved');setFileError(e.message);});},500);
    return () => clearTimeout(draftTimer.current);
  },[draft,content,fileData]);
  useEffect(() => {let active = true;api(`/projects/${id}/drafts`).then(value => {if (active) setStoredDrafts(value.drafts);}).catch(e => {if (active) setError(`Saved drafts could not be checked: ${e.message}`);});return () => {active = false;};},[id,project]);
  async function moreFiles() {
    if (filePage?.nextCursor == null || fileBusy) return;
    setFileBusy(true);setError('');
    try {const value = await api(`/projects/${id}/files?cursor=${filePage.nextCursor}&limit=100`);
      if (value.revision !== filePage.revision) {setFilePage(await api(`/projects/${id}/files`));setNotice('The file list changed. It has been refreshed.');}
      else setFilePage(current => ({...value,files:[...current.files,...value.files]}));
    } catch(e) {setError(e.message);} finally {setFileBusy(false);}
  }
  useEffect(() => {let active = true;api(`/projects/${id}/files`).then(value => {if (active) setFilePage(value);}).catch(e => {if (active) setError(e.message);});return () => {active = false;};}, [id, project]);
  useEffect(() => {let active = true;api(`/projects/${id}/adoption`).then(value => {if (active) setAdoption(value);}).catch(() => {});return () => {active = false;};}, [id]);
  async function load() {
  ...
  useEffect(() => {
    ...
    let active = true;setReadBusy(true);setFileError('');setFileData(null);
    Promise.all([api(`/projects/${id}/content?path=${encodeURIComponent(file)}`).catch(e => ({loadError:e.message})),api(`/projects/${id}/draft?path=${encodeURIComponent(file)}`)]).then(([value,saved]) => {if (value.loadError && !saved) throw new Error(value.loadError);if (active) {setReadBusy(false);draftRevision.current = saved?.revision ?? null;setFileData(saved ? {...value,path:file,content:saved.baseContent,readonly:false} : value);setContent(saved ? saved.content : value.content);setDraft(!!saved);setDraftStatus(saved ? 'Recovered a draft saved on this Mac' : '');if (saved?.stale) setFileError('The file changed after this draft began. Your draft is preserved; writing will require resolving that change.');}}).catch(e => {if (active) setFileError(e.message);}).finally(() => {if (active) setReadBusy(false);});
    return () => {active = false;};
  }, [id, file, draft, fileRefresh, tab]);
  async function act(action, success) {
    setBusy(true); setError(''); setNotice('');
    let completed = false;
    try { const result = await action(); completed = true; setDialog(null); setNotice(success); if (result?.files) setProject(result); else await load(); setFileRefresh(value => value + 1); onUpdated().catch(() => {}); }
  ...
  function chooseFile(next) { if (draft || busy || fileBusy) return; setFile(next); }
  async function writeFile() {
    await act(async () => {
      const saved = await persistDraft(content);
      const result = await api(`/projects/${id}/file`, {path: file, content, expectedContent: fileData?.content ?? null});
      try {await api(`/projects/${id}/draft/discard`,{path:file,revision:saved.revision});draftRevision.current = null;setDraft(false);setDraftStatus('');}
      catch(e) {throw new Error(`The file was written, but its saved draft could not be cleared: ${e.message}`);}
      return result;
    }, 'File written to this computer. Save a version to add it to history.');
  }
  async function submit(event) {
    event.preventDefault();
    if (dialog === 'discard') { setBusy(true); try { clearTimeout(draftTimer.current);await draftQueue.current.catch(() => {});if (draftRevision.current) await api(`/projects/${id}/draft/discard`,{path:file,revision:draftRevision.current});draftRevision.current = null;setDraftStatus('');await load(); setDraft(false); setFileRefresh(value => value + 1); setDialog(null); setError(''); setNotice('Editor reloaded from the file on this computer.'); } catch(e) {setError(e.message);} finally {setBusy(false);} return; }
  ...
  const readonly = fileData?.readonly;
  ...
  return <>
    ...
    {tab === 'Files' && <>
      ...
      <div className="editor-layout"><nav className="file-list" aria-label="Project files">{(filePage?.files || []).map(f => <button key={f.path} className={f.path === file ? 'selected' : ''} disabled={busy || (draft && f.path !== file)} onClick={() => chooseFile(f.path)}><Icon name="file" size={16}/><span>{f.path}</span></button>)}<p className="note">{filePage ? `${filePage.files.length} of ${filePage.total} files shown` : 'Loading file list…'}</p>{filePage?.nextCursor != null && <button disabled={fileBusy || draft || busy} onClick={moreFiles}>{fileBusy ? 'Loading…' : 'Show more files'}</button>}</nav>
        <div className="editor"><label htmlFor="file-content">{file}{readonly ? ' · Read only' : ''}</label>{fileError && <p role="alert" className="error">{fileError}</p>}{readBusy && <p role="status">Opening file…</p>}<textarea id="file-content" spellCheck={false} value={fileData || draft ? content : ''} readOnly={readonly} disabled={busy || readBusy || !fileData} onChange={e => {setContent(e.target.value); setDraft(true);}}/><div className="editor-actions"><span className="note" role="status">{draftStatus || 'Writing updates the file. Saving a version records its history.'}</span>{draft && <button className="text-button" disabled={busy} onClick={() => show('discard')}>Reload file</button>}<button disabled={!draft || busy || readonly || readBusy || !fileData} onClick={writeFile}>{busy ? 'Writing…' : 'Write file'}</button></div></div>
      </div>
    ...
    {tab === 'History' && <>
      ...
      <ol className="history">{(historyPage?.history || project.history).map((version, index) => <li key={version.id}><span className="timeline-dot"/><div><h3>{version.message}</h3><p>{relativeDate(version.date)}<span className="technical"> · <code>{version.id.slice(0, 8)}</code></span>{index === 0 ? ' · Current version' : ''}</p></div>{index > 0 && <button className="secondary" disabled={busy || project.dirty} onClick={() => show({revision: version.id, message: version.message})}>Restore</button>}</li>)}</ol>{historyPage?.nextCursor != null && <button className="secondary" disabled={historyBusy || busy} onClick={olderHistory}>{historyBusy ? 'Loading…' : 'Load older versions'}</button>}
    </>}
    {tab === 'Dependencies' && <Footprint id={id}/>}
    {tab === 'Take it with you' && <div className="export-view"><h2>No permission needed to leave.</h2><p>Download your committed files, requests, and complete Git history in a portable bundle.</p>{project.dirty && <p className="notice">Your latest file changes are not in history yet. Save a version before exporting.</p>}<a className={`button ${project.dirty ? 'disabled' : ''}`} aria-disabled={project.dirty} href={project.dirty ? undefined : `/api/projects/${id}/export`} download><Icon name="export"/>Download project history</a><h3>Open it without Unforge</h3><code className="command">git clone your-project.bundle my-project</code><p>Or use “Bring an existing app” on another Unforge workspace.</p>{adoption && <><h3>Your original and this copy</h3><code className="command">{adoption.sourcePath}</code><p>This is an independent copy. Later changes do not synchronize automatically.</p><details><summary>Import record and omissions</summary><pre style={{whiteSpace:'pre-wrap',overflowWrap:'anywhere'}}>{JSON.stringify({history:adoption.git,omitted:adoption.skipped},null,2)}</pre></details></>}<h3>Your working folder</h3><code className="command">{project.path}</code><p className="note">Keep the bundle on another device for an independent copy. It excludes unwritten editor changes, uncommitted files, external databases, separately stored credentials, and external large-file objects. Any secrets already committed remain in history; export is not a secret scanner.</p></div>}
